...
class TetrisApp:
    def help(self):
        ...
        self.canvas.create_text(100,50,text='TETRIS', font=('나눔고딕',24), anchor='center', fill='orange')
        self.canvas.create_text(10,100,text='Up: rotate', font=('나눔고딕',16), anchor='w', fill='white')
        self.canvas.create_text(10,130,text='Left: move left', font=('나눔고딕',16), anchor='w', fill='white')
        self.canvas.create_text(10,160,text='Right: move right', font=('나눔고딕',16), anchor='w', fill='white')
        self.canvas.create_text(10,190,text='Down: move down', font=('나눔고딕',16), anchor='w', fill='white')
        self.canvas.create_text(10,220,text='p: pause', font=('나눔고딕',16), anchor='w', fill='white')
        self.canvas.create_text(10,250,text='Space: start', font=('나눔고딕',16), anchor='w', fill='white')
    ...
    def check_collision(self,offset):
        ...
        for (y, row) in enumerate(self.stone):          # (y,row) = (0, [0,0,2]), (1,[2,2,0])
            for (x, val) in enumerate(row):             # (x,val) = (0,0), (1,2), (2,2)
                if 0 <= y + off_y < self.rows and 0 <= x + off_x < self.cols:       # 유효한 인덱스인지를 검사
    ...
    def new_stone(self):
        ...
        self.stone = tetris_shapes[random.randrange(len(tetris_shapes))]        # 0,1,...,6 중에서 랜덤하게 선택함
        ...
        if self.check_collision((self.stone_x, self.stone_y)):          # self.stone 이 self.board 의 다른 블록과 충돌이 하는지를 검사!
            self.gameover =True
    ...
    def check_remove(self):
        ...
        i = self.rows - 1           # 맨 밑에 줄 부터!
        while i >= 0:               # i = rows - 1, .... , 0
            # self.gridCell= [[1,2,1,2,1,2],[0,1,0,1,2,1,1],...]
            if not 0 in self.gridCell[i]:    # i행에 0이 없으면!
                del (self.gridCell[i])       # i번째 행 원소를 삭제
                # self.gridCell 앞에 [0,0,0,....,0]을 붙임
                self.gridCell =[[0 for _ in range(self.cols)]] + self.gridCell
            else:   # i번째 행이 채워지지 않았으면 i를 1만큼 감소
                i -= 1
    def move_down(self):
        if self.check_collision((self.stone_x, self.stone_y + 1)):      # stone을 한 칸 아래로 움직일 때 충돌 검사!
            self.merge_stone()          # 현재 stone을 board에 추가해서 고정시킴
            self.new_stone()            # 새로운 stone 생성
            self.check_remove()         # 채워진 줄은 지움

        else:           # 충돌이 일어나지 않았으면..
            self.stone_y += 1           # stone은 1칸 아래로 이동

        self.paintGrid()

    def move_left(self):
        if not self.check_collision((self.stone_x - 1, self.stone_y)):      # 왼쪽으로 움직여서 충돌 검사!
            self.stone_x -= 1
            self.paintGrid()

    def move_right(self):
        if not self.check_collision((self.stone_x + 1, self.stone_y)):      # 오른쪽으로 움직여서 충돌 검사!
            self.stone_x += 1
            self.paintGrid()
    ...
    def __init__(self,w,h):
        ...
        self.help() #키 입력 설명
        self.init_game()
        self.window.bind('<Key>',self.link_keys) # 키 입력 이벤트 함수 self.link_keys 연결
